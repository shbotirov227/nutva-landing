...
export function FormModal({ children }: FormModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isNameEmpty) {
      toast.error(t("form.errors.nameRequired") || "Ism kiritilishi shart");
      return;
    }

    if (isPhoneInvalid) {
      toast.error(t("form.errors.phoneRequired") || "Telefon raqami to'liq bo'lishi kerak");
      return;
    }

    // Hamma narsa to‘g‘ri bo‘lsa
    toast.success(t("form.success") || "So‘rovingiz muvaffaqiyatli yuborildi!");
    console.log("Form yuborildi:", { name, phone });
    ...
    <>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <motion.div
              className="bg-white text-black rounded-2xl w-full max-w-md sm:max-w-lg md:max-w-xl lg:max-w-2xl xl:max-w-xl mx-auto p-6 sm:p-8 md:p-10 border-none"
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="grid gap-4">
                <input
                  ...
                  className="w-full px-4 py-3 sm:px-5 sm:py-3 rounded-xl text-[#6F6F6F] text-[15px] font-bold bg-white outline-none border-2 border-[#FD902B] focus:shadow-[0_0_10px_rgba(253,144,43,0.8)] transition-all"
                />
